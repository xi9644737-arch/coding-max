...
<p align="center">
  ...
  <img alt="Spec" src="https://img.shields.io/badge/spec-agentskills.io-green">
</p>
...
`coding-max` 是一套让 AI **直面根因**的方法论。4 种模式、9 步流程、10 条硬约束、7 个自欺红旗。从"线上炸了赶紧救"到"这个报错帮我看一眼"，一条流程覆盖到底。
...
这是 **skill 文件**，不是 JS 包。它只包含 Markdown + YAML，遵循 [agentskills.io](https://agentskills.io) 规范。Claude Code 原生加载，不需要 `npm install`。
...
| 0.5 存档 | 建 Bug 报告 `.project-memory/bugs/` | Hotfix 跳过 |
| 1. 搜病历 | 查历史 Bug 模式，命中直接复用修复方案 | 首次使用 |
...
| 5. 墓碑 | 清理临时调试代码，独立 commit ≤5 个 | Hotfix 跳过 |
| 6. 全量验证 | 跑全量测试 + 覆盖率基线比对（无测试项目降级为手动清单） | — |
