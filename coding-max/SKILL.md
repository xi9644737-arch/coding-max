...
### 0.0 环境探测(首次,缓存)
有测试→步骤6正常 | 无测试→拦截:"项目为测试荒漠,是否搭基建?[Y/N]"→Y:写PHASE(`bootstrap`)→启动`coding-pipeline`→回步骤6 | 高风险→先确认
...
## 项目记忆
`BUG_PATTERNS`(病历)/`PROJECT_PROFILE`(画像+增量覆盖率基线)/`bugs/`(报告+胶囊)/`.resume`(断点)/`PHASE`(多skill状态锁)。格式见`references/bug-memory-format.md`,模板见`memory-template/`。
