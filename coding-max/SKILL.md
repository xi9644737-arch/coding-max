...
| 影响面 | 含义 | 分级倾向 |
|--------|------|---------|
| 单机 | CLI工具、脚本、本地服务 | 倾向Quick |
| 单用户 | Web请求、API单次调用 | Quick/Standard |
| 多用户 | 并发请求、共享状态 | 倾向Standard |
| 分布式 | 多服务、消息队列、数据库 | Standard/Complex |
...
| 模式 | 行为 |
|------|------|
| Trivial | **跳过**。拼写/注释/格式化不因缺测试而阻塞。 |
| Quick | **警告不阻断**。提示"无测试,修完后建议搭基建",继续流程。事后补:步骤7提示搭基建。 |
| Standard | **拦截**。写PHASE.json(`bootstrapping`)→启动`coding-pipeline`→回步骤6。 |
| Hotfix | **跳过**。事后补基建。 |
...
## 硬约束
1.RED先于GREEN 2.补丁+红旗→重做 3.禁裸吞异常(`except:`/`catch{}`/`recover()`) 4.临时物步骤5清 5.只改冲击波范围 6.arch-不重构(最小安全修复) 7.不确定=说不确定 8.写记忆需确认 9.不pip/push/rm 10.≥3失败硬停 11.**Quick是默认**,不因无测试而阻塞Trivial/Quick 12.影响面驱动分级,不看代码行数
...
## 项目记忆
`BUG_PATTERNS`(病历)/`PROJECT_PROFILE`(画像+增量覆盖率基线)/`bugs/`(报告+胶囊)/`.resume`(断点)/`PHASE.json`(多skill状态机,JSON格式)。格式见`references/bug-memory-format.md`,模板见`memory-template/`。

### PHASE.json 状态机
```json
{
  "state": "idle|bootstrapping|testing|done",
  "target": "<目标标识>",
  "skill": "coding-max|coding-pipeline",
  "started_at": "<ISO时间>",
  "updated_at": "<ISO时间>",
  "retry": 0
}
```
状态流转:`idle`→(写bootstrap)→`bootstrapping`→(管道通)→`testing`→(验证通过)→`done`。多skill通过`skill`字段区分,不相冲突。`retry`≥3→`done`(失败)。
