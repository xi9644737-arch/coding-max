...
修复读 incident；`failure-confirmed` 后、`localized` 前必须执行一次历史匹配：无 `BUG_PATTERNS.md` 写 `unavailable`，否则读检索协议；再读 workflow/格式，完成 Review。范围外问题获准才处理。
...
- 异常须处理、传播或记录；不越界重构，不做未授权 push/安装/破坏操作。
- 状态只由所需证据推进；Human Gate 未决须停在安全状态。
